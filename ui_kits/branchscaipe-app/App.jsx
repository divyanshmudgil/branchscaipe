...
function App() {
  ...
  const { Dialog, Button, Toast, IconButton, Tooltip } = NS;
  ...
  const contextTopics = () => branch
    ? BSC.contextMessages(branches, activeId).filter((m) => m.role === "assistant" && m.topic).map((m) => m.topic)
    : [];
  ...
  const send = (text) => {
    ...
    const userMsg = { id: BSC.uid("m"), role: "user", text: t };
    ...
    replyTimer.current = setTimeout(() => {
      ...
      if (!cur) return;
      const ctxTopics = BSC.contextMessages(branchesRef.current, targetId)
        .filter((m) => m.role === "assistant" && m.topic).map((m) => m.topic);
      const reply = BSC.generateReply({ branch: cur, userText: t, contextTopics: ctxTopics, merged: cur._lastMerge });
      setBranches(bs => {
        const b = bs[targetId];
        if (!b) return bs;
        return { ...bs, [targetId]: { ...b, messages: [...b.messages, { id: BSC.uid("m"), role: "assistant", text: reply }] } };
      });
      setStatus("idle");
  ...
  const retry = (msg) => {
    ...
    replyTimer.current = setTimeout(() => {
      ...
      if (!cur) return;
      const reply = BSC.generateReply({ branch: cur, userText: userText || "continue", contextTopics: contextTopics(), retry: true });
  ...
  const doMerge = (m) => {
    ...
    const divider = { id: BSC.uid("mg"), role: "merge", source: src.name, sourceId: m.sourceId, scope: m.scope, ts };
    let inserts;
    if (m.scope === "response") {
      inserts = [{ ...m.sourceMsg, id: BSC.uid("m"), starred: false, fromMerge: m.sourceId }];
    } else {
      inserts = src.messages
        .filter((x) => x.role === "user" || x.role === "assistant")
        .map((x) => ({ ...x, id: BSC.uid("m"), starred: false, fromMerge: m.sourceId }));
    }
