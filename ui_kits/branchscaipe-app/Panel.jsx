...
function Panel({ kind, branches, rootId, activeId, onSelect, onClose, onRename, onContextMenu, starred, onJumpStar, onUnstar, editRequestId, onEditConsumed }) {
      ...
      {starred.map((s) => (
        <div key={s.messageId} style={{ position: "relative", padding: "12px 12px 11px", background: "var(--surface-2)", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-lg)", cursor: "pointer" }}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
            ...
            <span style={{ flex: 1 }} />
            <span style={{ fontFamily: "var(--font-sans)", fontSize: "var(--fs-micro)", color: "var(--text-muted)" }}>{BSC.relTime(s.ts)}</span>
          </div>
        ...
        return (
          ...
          onMouseLeave={(e) => { if (!active) e.currentTarget.style.background = "transparent"; }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, width: "100%" }}>
              ...
              <span style={{ flex: "none", fontFamily: "var(--font-sans)", fontSize: "var(--fs-micro)", color: "var(--text-muted)" }}>{BSC.relTime(b.createdAt)}</span>
            </div>
